...
export class ListarComponent implements OnInit, AfterViewInit {
  delete!: boolean;
  constructor( private readonly reglaComercialService: ReglaComercialService, private readonly router: Router, private dialog: MatDialog,) {
  ...
  getReglas() {
    this.reglaComercialService.getReglas().subscribe((response: any) => {
  ...
  goToEditPage(idTipoReglaComercial: number) {
    this.router.navigateByUrl(`/regla-comercial/modificar/${idTipoReglaComercial}`);
  }
  ...
  openDialog(id: number): void {
    ...
    dialogRef.afterClosed().subscribe((result: boolean) => {
      if (result) {
        this.reglaComercialService.deleteRegla(id).subscribe(response => {
          setTimeout(() => {
            location.reload();
          }, 100);
        });
