...
export class ListarComponent implements OnInit, AfterViewInit {
  delete!: boolean;
  constructor( private readonly ivaService: IvaService, private readonly router: Router, private dialog: MatDialog,) {
  ...
  getIVAs() {
    this.ivaService.getIVAs().subscribe((response: any) => {
  ...
  goToEditPage(id: number) {
    this.router.navigateByUrl(`/iva/modificar/${id}`);
  }
  ...
  openDialog(id: number): void {
    ...
    dialogRef.afterClosed().subscribe((result: boolean) => {
      if (result) {
        this.ivaService.deleteIVA(id).subscribe(response => {
          setTimeout(() => {
            location.reload();
          }, 100);
        });
